...
This automatically updated document provides a real-time overview of a number of metrics that are central to economic and monetary policy in the EU. All data comes from the [ECB API](https://data.ecb.europa.eu/help/api/) and Eurostat. The document is current as of ```r date_now_f```.
...
As most government borrowing in the EU takes place at the national level, bond yields vary not only over time but also systematically by country. At the moment, yields are highest on bonds issued by ```r cur_max_yld$name```. Last month they traded with a yield of ```r cur_max_yld$values```%. In contrast, ```r cur_min_yld$name``` has the lowest yields at ```r cur_min_yld$values```%.
...
iplot_rates <- rates |> 
  ...
  geom_step_interactive(aes(color = name, tooltip = tooltip, data_id = name),
                        linewidth = 0.29,
                        show.legend = FALSE) +
  annotate(geom = "text", label = "2% inflation target", x = as.Date("1997-01-01"), y = 2.3,
           fontface = "italic", size = 2.7) +
